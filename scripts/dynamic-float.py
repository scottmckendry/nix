...
class Rule:
    """
    A rule to match a window. A window matches the rule if it matches any of the
    "match" patterns and none of the "exclude" patterns.
...
def float(id: int):
    ...
    send({"Action": {"SetWindowWidth": {"id": id, "change": {"SetFixed": 1200}}}})
    # short delay to allow resize propagation before centering
    time.sleep(0.05)
    send({"Action": {"CenterWindow": {"id": id}}})


def update_matched(win: Dict[str, Any]) -> None:
    win["matched"] = False
